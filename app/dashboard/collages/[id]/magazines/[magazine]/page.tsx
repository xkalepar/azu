...
export async function generateMetadata({
  ...
  params: { magazine: string };
}): Promise<Metadata> {
  const magazine = await getMagazine({ id: params.magazine });
  if (!magazine) {
    return {
      title: "404 غير موجود",
    };
  ...
  return {
    title: magazine.arContent?.title ?? `info about ${params.magazine}`,
  };
}

const newsPage = async ({ params }: { params: { magazine: string } }) => {
