...
export const createCenter = async ({
  ...
  try {
    const newCollage = await prisma.graduatesForUniversity.create({
      data: {
        arContent: {
          create: arContent,
        },
...
export const editCenter = async ({
  ...
  try {
    const newCollage = await prisma.graduatesForUniversity.update({
      where: { id: centerId },
      data: {
        arContent: {
          update: {
...
export const deleteCenter = async ({
  ...
  try {
    const news = await prisma.graduatesForUniversity.delete({
      where: { id },
    });
...
export const getCenters = unstable_cache(
  async () => {
    try {
      const graduatesForUniversity =
        await prisma.graduatesForUniversity.findMany({
          include: { arContent: true, enContent: true },
        });
      if (!graduatesForUniversity) {
        return [];
      }
      return graduatesForUniversity;
    } catch (error) {
      return [];
    }
  },
