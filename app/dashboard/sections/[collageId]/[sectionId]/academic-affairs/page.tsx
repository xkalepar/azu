    ...
    <section className=" relative ">
      <div className="flex justify-between gap-1 items-start sm:flex-row flex-col">
        ...
        >
          ...
          <BreadcrumbSeparator />
          <FinalBreadcrumbItem>{"الشؤون االكاديمية"}</FinalBreadcrumbItem>
        </Breadcrumbs>
        <Link
          href={"academic-affairs/new"}
          className={cn(
      ...
      <div className="grid my-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-1 sm:gap-3 xl:gap-4">
        {section?.AcademicAffairs?.map((department, index) => (
          <ListItem href={`academic-affairs/${department.id}`} key={index}>
