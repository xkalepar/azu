    ...
    <section className=" relative ">
      <div className="flex justify-between gap-1 items-start sm:flex-row flex-col">
        ...
        >
          ...
          <BreadcrumbSeparator />
          <FinalBreadcrumbItem>{"نماذج وادلة القسم"}</FinalBreadcrumbItem>
        </Breadcrumbs>
        <Link
          href={"department-forms-and-guidelines/new"}
          className={cn(
      ...
      <div className="grid my-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-1 sm:gap-3 xl:gap-4">
        {section?.DepartmentFormsAndGuidelines?.map((department, index) => (
          <ListItem
            href={`department-forms-and-guidelines/${department.id}`}
            key={index}
          >
