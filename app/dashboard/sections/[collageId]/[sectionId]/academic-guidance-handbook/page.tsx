    ...
    <section className=" relative ">
      <div className="flex justify-between gap-1 items-start sm:flex-row flex-col">
        ...
        >
          ...
          <BreadcrumbSeparator />
          <FinalBreadcrumbItem>{"دليل االإرشاد الأكاديمي"}</FinalBreadcrumbItem>
        </Breadcrumbs>
        <Link
          href={"academic-guidance-handbook/new"}
          className={cn(
      ...
      <div className="grid my-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-1 sm:gap-3 xl:gap-4">
        {section?.AcademicGuidanceHandbook?.map((department, index) => (
          <ListItem
            href={`academic-guidance-handbook/${department.id}`}
            key={index}
          >
